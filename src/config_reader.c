...
* returns -1 on failure to find the index of search_term char in the string.
* In order for this function to work properly the given string has to null-terminated.
*/
ssize_t split_index(const char *str, const char search_term){
...
*/
int8_t copy_str_after_split(const char *src, String **target_buffer, const char splitter){
    ssize_t split_idx=split_index(src, splitter);
    if (split_idx<0){
        fprintf(stderr, "%sThe splitting character was not found in the source string.\n", ERROR_LOG_STR);
        return -1;
    }
    // Incase split character is the last character in the line
    if (split_idx==strlen(src)-1){    
        fprintf(stderr, "%sThere is nothing after the split character in src string: '%s'.\n", ERROR_LOG_STR, src);
        return -1;
    }
    *target_buffer=init_string_from_src(src+split_idx+1);
    if (target_buffer==NULL){
        fprintf(stderr, "%sError allocating the destination buffer. Unable to copy the split string.\n", ERROR_LOG_STR);
        return -1;
    }
    return 0;
}
...
int8_t add_new_mapping(AudioMappings *configs, const char *line){
    if (configs==NULL){
        fprintf(stderr, "%sUninitialized AudioMappings data structure in configs parameter.\n", ERROR_LOG_STR);
        return -1;
    }
    int8_t input_number=atoi(line); 
    if (input_number>configs->total_number_of_inputs || input_number<=0){
        fprintf(stderr, "%sPlease provide valid number to map audio file to: %s\n", ERROR_LOG_STR, line);
        return -1;
    }
...
AudioMappings * init_audio_mapping(const char *config_filename, const uint8_t number_of_inputs){
    ...
    if (newobj==NULL){
        fprintf(stderr, "%sAudioMapping allocation failed. Error: %s\n", ERROR_LOG_STR, strerror(errno));
        return NULL; 
    }
    ...
    if (newobj->audio_mapping_arr==NULL){
        fprintf(stderr, "%sString buffer allocation failed during AudioMapping initialization. Error: %s\n", ERROR_LOG_STR, strerror(errno));
    ...
    if (newobj->is_audio_map_changed==NULL){
        fprintf(stderr, "%sUnable to allocate the is_audio_map_changed flag buffer. Error: %s\n", ERROR_LOG_STR, strerror(errno));
        deinit_string(&(newobj->filename));
...
uint8_t generate_config(){
	...
	if (fputs(default_config, baseconfigfile)<0){
		fprintf(stderr, "%sIssue generating the default template for the config file: %s\n", ERROR_LOG_STR, strerror(errno));
		return -1;
	}
	return 0;
}
...
int8_t parse_config_file(AudioMappings *configmap){
    ...
    if (config_file==NULL){
        fprintf(stderr, "%sOpening config failed. Error: %s\n", ERROR_LOG_STR, strerror(errno));
        return -1;
    }
    ...
    if (buffer==NULL){
        fprintf(stderr, "%sError allocating string buffer for reading context file. Error: %s\n", ERROR_LOG_STR, strerror(errno));
        return -1;
    }
    ssize_t linesize=0;
...
int8_t is_modified(const AudioMappings *configs){
    if (configs==NULL){
        fprintf(stderr, "%sUninitialized configs data structure.\n", ERROR_LOG_STR);
        return -1;
    }
    struct stat fileparams;
    if (stat(configs->filename->str, &fileparams)!=0){ // Get the file attributes for the target config filename
        fprintf(stderr, "%sUnable to get the file attributes of the config file: %s\n", ERROR_LOG_STR, configs->filename->str);
        return -1;
    }
